...
\item Drove \textbf{release and patch qualification} across multiple platform variants (Compute, FI infra bundles from 4.2.x to 6.0.2.x), ensuring readiness for staging and production releases.
...
\item Contributed to \textbf{AI tools} such as (bug scribe, test plan generation, visionai), presenting solutions to internal teams and leadership.
...
\textbf{Software Engineer C, Data Center BU, Cisco} \hfill July 2022 -- July 2023

\quad \textbf{L3 Team}
\begin{compactitem}
\item Developed and implemented \textbf{CLI-based features} based on customer requirements across multiple release cycles.
\item Configured and debugged \textbf{L2/L3 networking issues} (ISIS, BFD, VRF, ARP) and forwarding behavior in lab deployments as part of regression runs and CFD.
\item Developed expertise in \textbf{control plane and inter-process communication} between supervisor and line cards especially in broadcom chips.
...
\item Set up and validated \textbf{Nexus switch topologies (fat-tree)} for testing and debugging in vega lab.
...
\item Built and debugged \textbf{containerized (Docker) network testing workflows} for emulator runs.
...
\item {\bf Networking:}
        \begin{innerlist}
        \item[$\diamond$] Constructed network using {\bf Mininet} with multiple {\bf TRex Traffic Generator} at hosts running stateless, stateful and Client Server L7 traffic and obtained telemetry.[2021]
       \item[$\diamond$] Constructed {\bf \href{https://github.com/bidhovbizar/DCN-Simulation/tree/master/PythonCode}{Simulation Framework}} in {\bf Python} for the traffic testing of large scale Data Center in Fat-Tree Topology. [2020]
       \item[$\diamond$] Emulated Fat-Tree topology in Mininet to collect the telemetry data and observe congestion using  {\bf Ryu} Controller and {\bf OpenVSwitches}.[2020]
